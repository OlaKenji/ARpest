...
#                zscale = zscale[0]
            ...
            # Special case for 'scangroup'
                ...
                #added this stuff
                xscale = []
                for str in list(points.split(",")):
                    if str[1] == '(':
                        xscale.append(float(str[2:-2]))
                xscale = np.array(xscale)
        ...
        for position in np.array(infile['/entry1/instrument/manipulator']):
            metadata[position] = np.array(infile['/entry1/instrument/manipulator/'+position])

        for ana in np.array(infile['/entry1/instrument/analyser']):
            metadata[ana] = np.array(infile['/entry1/instrument/analyser/'+ana])
...
#print(np.array(infile['/entry1/instrument/manipulator/sax']))#['saazimuth' 'sapolar' 'satilt' 'sax' 'say' 'saz']
...
class Dataloader_SIS(Dataloader) :
    ...
    def load_zip(self, filename) :
        ...
        res = Namespace(
            ...
            yscale = yscale,
            zscale = energies,
            metadata = {'Excitation Energy':M2.hv}
        )
        return res
